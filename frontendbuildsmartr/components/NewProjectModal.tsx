...
import { X, File, Trash2, HardHat, Ruler, FileText, Loader2, AlertCircle, Mail, CheckCircle2 } from "lucide-react"
...
export function NewProjectModal({ isOpen, onClose }: NewProjectModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fire indexing request (fire and forget - don't wait)
      fetch(`/api/projects/${project.id}/index`, { method: 'POST' })
        .catch(err => console.error('Failed to start indexing:', err))
    ...
    return (
      <AnimatePresence>
        ...
        <motion.div
          ...
          className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-[#111827] border border-border rounded-xl shadow-2xl z-50 p-8 text-center"
        >
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg max-h-[90vh] bg-[#111827] border border-border rounded-xl shadow-2xl z-50 flex flex-col"
          >
            {/* Header */}
