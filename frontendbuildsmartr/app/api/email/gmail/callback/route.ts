...
export async function GET(request: NextRequest) {
  ...
  // Forward any errors from Google
  if (error) {
    return NextResponse.redirect(`/account?error=gmail_auth_failed&message=${error}`)
  }

  if (!code) {
    return NextResponse.redirect('/account?error=gmail_auth_failed&message=missing_code')
  }
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/oauth/gmail/callback?${params.toString()}`, {
